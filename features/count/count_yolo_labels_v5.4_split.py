...
# ---- CONFIG ----
DATA_YAML_PATH = "../../dataset/v5.4/data.yaml"
...
plt.title("Total Distribution (v5.4)")
...
for split in SPLITS:
    ...
    plt.figure(figsize=(14, 6))
    plt.bar(labels, values)
    plt.title(f"{split.upper()} Distribution (v5.4)")
    plt.xticks(rotation=45)
    plt.grid(axis="y", linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.show()
...
for split in SPLITS:
    ...
    plt.figure(figsize=(14, 6))
    plt.bar(labels, values)
    plt.title(f"Normalized Distribution — {split.upper()} (v5.4)")
    plt.grid(axis="y", linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.show()
...
plt.title("Normalized Distribution Comparison (v5.4)")
plt.xlabel("Class")
plt.ylabel("Ratio")
plt.legend()

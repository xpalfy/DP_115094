...
DATA_YAML_PATH = f"../../dataset/{VERSION}/data.yaml"
...
def plot_total_distribution(total_counts, class_map, nc):
    ...
    plt.title(f"Total Distribution ({VERSION})")
...
def plot_split_distributions(counts_by_split, class_map, nc):
    for split in SPLITS:
        ...
        plt.figure(figsize=(14, 6))
        plt.bar(labels, values)
        plt.title(f"{split.upper()} Distribution ({VERSION})")
        plt.xticks(rotation=45)
        plt.grid(axis="y", linestyle="--", alpha=0.5)
        plt.tight_layout()
        plt.show()
...
def plot_normalized_distributions(counts_by_split, nc):
    for split in SPLITS:
        ...
        plt.figure(figsize=(14, 6))
        plt.bar(labels, values)
        plt.title(f"Normalized Distribution — {split.upper()} ({VERSION})")
        plt.grid(axis="y", linestyle="--", alpha=0.5)
        plt.tight_layout()
        plt.show()
...
def plot_overlay(counts_by_split, nc):
    ...
    plt.title(f"Normalized Distribution Comparison ({VERSION})")
    plt.xlabel("Class")
    plt.ylabel("Ratio")
    plt.legend()

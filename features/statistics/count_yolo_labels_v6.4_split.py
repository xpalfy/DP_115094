...
DATA_YAML_PATH = "../../dataset/v6.4/data.yaml"
...
def plot_total_distribution(total_counts, class_map, nc):
    ...
    plt.title("Total Distribution (v6.4)")
...
def plot_split_distributions(counts_by_split, class_map, nc):
    for split in SPLITS:
        ...
        plt.figure(figsize=(14, 6))
        plt.bar(labels, values)
        plt.title(f"{split.upper()} Distribution (v6.4)")
        plt.xticks(rotation=45)
        plt.grid(axis="y", linestyle="--", alpha=0.5)
        plt.tight_layout()
        plt.show()
...
def plot_normalized_distributions(counts_by_split, nc):
    for split in SPLITS:
        ...
        plt.figure(figsize=(14, 6))
        plt.bar(labels, values)
        plt.title(f"Normalized Distribution — {split.upper()} (v6.4)")
        plt.grid(axis="y", linestyle="--", alpha=0.5)
        plt.tight_layout()
        plt.show()
...
def plot_overlay(counts_by_split, nc):
    ...
    plt.title("Normalized Distribution Comparison (v6.4)")
    plt.xlabel("Class")
    plt.ylabel("Ratio")
    plt.legend()
